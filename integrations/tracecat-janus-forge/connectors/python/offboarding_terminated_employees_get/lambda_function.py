...
def fetch_terminated_employees(since_days: int, base_url: str, auth_type: str, token: str, username: str = "", password: str = "") -> List[Dict[str, Any]]:
    url = base_url.rstrip("/") + "/api/grc/terminated"
    ...
    r = requests.get(url, headers=headers, params=params, timeout=30, auth=auth)
    ...
    for e in data:
        items.append({
            ...
            "email": e.get("email"),
            "terminationDate": e.get("terminationDate"),
        })
    return items
...
def lambda_handler(event, _context):
    ...
    since_days = int(params.get("sinceDays", 1))
    try:
        ...
        if "bearer" in secret:
            items = fetch_terminated_employees(since_days, base_url, "bearer", token=secret["bearer"])
        else:
            items = fetch_terminated_employees(since_days, base_url, "basic", token="", username=secret.get("username", ""), password=secret.get("password", ""))
