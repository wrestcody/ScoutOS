...
# Security: all secrets in AWS Secrets Manager; no secrets in logs.
...
def bedrock_generate_query(nl_request: str, source_category: str) -> str:
    ...
    prompt = (
        "You are The Augur, an expert in Sumo Logic Search Query Language. "
        "Identify the user's intent (e.g., user role changes, failed logins, permission changes) and generate a valid Sumo query. "
        f"Assume logs are in source category '{source_category}'. "
        "Return ONLY the query string, nothing else.\n\nAudit Request: " + nl_request
    )
    body = {
